...
# 
# 用法：
#   ./test-server.sh          # 测试本地服务器 (localhost:3100)
#   ./test-server.sh prod     # 测试生产环境 (https://chu3.top)

# 检查参数
if [ "$1" = "prod" ] || [ "$1" = "production" ]; then
  MODE="production"
  BASE_URL="https://chu3.top"
  echo "╔═══════════════════════════════════════════════════╗"
  echo "║   🧪 Brew Guide - 生产环境测试                   ║"
  echo "╚═══════════════════════════════════════════════════╝"
else
  MODE="local"
  BASE_URL="http://localhost:3100"
  echo "╔═══════════════════════════════════════════════════╗"
  echo "║   🧪 Brew Guide Server - 本地功能验证            ║"
  echo "╚═══════════════════════════════════════════════════╝"
fi
...
if [ "$MODE" = "local" ]; then
  ...
  # 如果没有 .env 文件，使用默认值
  if [ -z "$ADMIN_KEY" ]; then
    ADMIN_KEY="brew-guide-admin-2025"
  fi
fi
