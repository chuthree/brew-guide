...
const StatsCategories: React.FC<StatsCategoriesProps> = ({
  ...
  // 重置计数器
  categoryCounter.current = 0;

  // 获取下一个序号的函数
  const getNextNumber = () => {
    categoryCounter.current += 1;
    return categoryCounter.current;
  };
    ...
    <div className="flex flex-col gap-8">
      <div className="grid grid-cols-1 gap-8">
        ...
        >
          ...
          <StatItem
            label="剩余重量"
            value={`${formatNumber(stats.remainingWeight)}`}
            unit="克"
          />
          <StatItem
            label="已消耗重量"
            value={`${formatNumber(stats.consumedWeight)}`}
            unit="克"
          />
          <StatItem
            label="消耗比例"
            value={`${stats.totalWeight > 0 ? formatNumber((stats.consumedWeight / stats.totalWeight) * 100) : '0'}`}
            unit="%"
          />
          <StatItem
            label="今日消耗"
            value={`${formatNumber(todayConsumption)}`}
            unit="克"
          />
        </StatCategory>
        ...
        >
          ...
          <StatItem
            label="剩余咖啡价值"
            value={`${formatNumber(stats.remainingWeight * stats.averageGramPrice)}`}
            unit="元"
          />
          <StatItem
            label="已消耗咖啡价值"
            value={`${formatNumber(stats.consumedWeight * stats.averageGramPrice)}`}
            unit="元"
          />
          <StatItem
            label="平均每包价格"
            value={`${formatNumber(stats.averageBeanPrice)}`}
            unit="元"
          />
          <StatItem
            label="每克平均价格"
            value={`${formatNumber(stats.averageGramPrice)}`}
            unit="元/克"
          />
          <StatItem
            label="今日花费"
            value={`${formatNumber(todayCost)}`}
            unit="元"
          />
        </StatCategory>
                ...
                {espressoTotal > 0 && (
                  <StatItem
                    ...
                    value={(() => {
                      const active = beans.filter(
                        bean =>
                          bean.beanType === 'espresso' && !isBeanEmpty(bean)
                      ).length;
        ...
        {stats.flavorPeriodStatus.inPeriod +
          ...
          >
            {stats.flavorPeriodStatus.inPeriod > 0 && (
              <StatItem
                label="在赏味期内"
                value={`${stats.flavorPeriodStatus.inPeriod}`}
                unit="个"
              />
            )}
            {stats.flavorPeriodStatus.beforePeriod > 0 && (
              <StatItem
                label="尚未进入赏味期"
                value={`${stats.flavorPeriodStatus.beforePeriod}`}
                unit="个"
              />
            )}
            {stats.flavorPeriodStatus.afterPeriod > 0 && (
              <StatItem
                label="已过赏味期"
                value={`${stats.flavorPeriodStatus.afterPeriod}`}
                unit="个"
              />
            )}
          </StatCategory>
        ...
        {stats.espressoStats.totalBeans > 0 && (
          ...
          >
            ...
            <StatItem
              label="正在使用"
              value={`${stats.espressoStats.activeBeans}`}
              unit="个"
            />
            <StatItem
              label="总重量"
              value={`${formatNumber(stats.espressoStats.totalWeight)}`}
              unit="克"
            />
            <StatItem
              label="剩余重量"
              value={`${formatNumber(stats.espressoStats.remainingWeight)}`}
              unit="克"
            />
            <StatItem
              label="已消耗重量"
              value={`${formatNumber(stats.espressoStats.consumedWeight)}`}
              unit="克"
            />
            <StatItem
              label="总花费"
              value={`${formatNumber(stats.espressoStats.totalCost)}`}
              unit="元"
            />
            <StatItem
              label="平均每包价格"
              value={`${formatNumber(stats.espressoStats.averageBeanPrice)}`}
              unit="元"
            />
            <StatItem
              label="每克平均价格"
              value={`${formatNumber(stats.espressoStats.averageGramPrice)}`}
              unit="元/克"
            />
            <StatItem
              label="今日消耗"
              value={`${formatNumber(stats.espressoStats.todayConsumption)}`}
              unit="克"
            />
            <StatItem
              label="今日花费"
              value={`${formatNumber(stats.espressoStats.todayCost)}`}
              unit="元"
            />
          </StatCategory>
        )}
        ...
        {stats.filterStats.totalBeans > 0 && (
          ...
          >
            ...
            <StatItem
              label="正在使用"
              value={`${stats.filterStats.activeBeans}`}
              unit="个"
            />
            <StatItem
              label="总重量"
              value={`${formatNumber(stats.filterStats.totalWeight)}`}
              unit="克"
            />
            <StatItem
              label="剩余重量"
              value={`${formatNumber(stats.filterStats.remainingWeight)}`}
              unit="克"
            />
            <StatItem
              label="已消耗重量"
              value={`${formatNumber(stats.filterStats.consumedWeight)}`}
              unit="克"
            />
            <StatItem
              label="总花费"
              value={`${formatNumber(stats.filterStats.totalCost)}`}
              unit="元"
            />
            <StatItem
              label="平均每包价格"
              value={`${formatNumber(stats.filterStats.averageBeanPrice)}`}
              unit="元"
            />
            <StatItem
              label="每克平均价格"
              value={`${formatNumber(stats.filterStats.averageGramPrice)}`}
              unit="元/克"
            />
            <StatItem
              label="今日消耗"
              value={`${formatNumber(stats.filterStats.todayConsumption)}`}
              unit="克"
            />
            <StatItem
              label="今日花费"
