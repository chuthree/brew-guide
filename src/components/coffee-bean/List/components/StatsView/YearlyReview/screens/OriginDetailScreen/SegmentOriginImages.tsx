...
interface SegmentOriginImagesProps {
  images: string[];
  onComplete?: () => void;
}
 ...
 * 产地图片展示 - 每张图片依次从右向左滑过
 */
const SegmentOriginImages: React.FC<SegmentOriginImagesProps> = ({
  images,
  onComplete,
}) => {
  ...
  const getLayoutInfo = () => {
    ...
    const imageSize = Math.floor((availableHeight - totalGaps) / images.length);
...
export default SegmentOriginImages;
