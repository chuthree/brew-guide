...
interface SegmentOriginHeadlineProps {
  originName: string;
  originImages: string[];
  onComplete?: () => void;
...
/**
 * 产地详情 - 第一段
 * 左上角文字"你是 {产地} 铁杆粉" + 两列图片往左下依次重叠
 * 复用 FavoriteRoasterScreen Segment2 的设计
 */
const SegmentOriginHeadline: React.FC<SegmentOriginHeadlineProps> = ({
  originName,
  originImages,
  onComplete,
  blurRef,
  ...
  const getRepeatedImages = (count: number) => {
    ...
    for (let i = 0; i < count; i++) {
      result.push(originImages[i % originImages.length]);
    }
    return result;
  };
  ...
  useGSAP(
    ...
    { scope: containerRef, dependencies: [allImages, originName] }
  );
...
export default SegmentOriginHeadline;
