...
export const exportListPreview = async (
    ...
    const backgroundColor = options.backgroundColor || (isDarkMode ? '#171717' : '#fafafa')
    ...
    try {
        ...
        // 获取实际渲染的组件
        const exportComponent = tempContainer.querySelector('.export-list-container') as HTMLElement
        if (!exportComponent) {
