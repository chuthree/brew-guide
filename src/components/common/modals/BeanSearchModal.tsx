...
const BeanSearchModal: React.FC<BeanSearchModalProps> = ({
  ...
  const [_searchCache, setSearchCache] = useState<Map<string, CoffeeBean[]>>(
    new Map()
  );

  // 全部咖啡豆数据
  const [allBeans, setAllBeans] = useState<CoffeeBean[]>([]);
  const [beansByBrand, setBeansByBrand] = useState<
    Record<string, CoffeeBean[]>
  >({});
  ...
  // 从缓存加载或获取数据
  const loadBeansData = useCallback(
      ...
      try {
        ...
        if (!forceRefresh) {
          ...
          const isRecentSession =
            lastOpenTime && now - lastOpenTime < SESSION_CACHE_DURATION;
          const isValidCache =
            cachedData && cachedExpiry && now < parseInt(cachedExpiry);
        ...
        // 获取咖啡豆数据
        const baseUrl =
          'https://gitee.com/chu3/brew-guide-bean-data/raw/master/coffee-beans-database.json';
        ...
        Object.keys(databaseData).forEach(brand => {
          ...
          const brandBeans = databaseData[brand].map(
            (bean: Record<string, unknown>): CoffeeBean => {
              // 构建咖啡豆对象，只设置确实有值的字段
              const coffeeBean: CoffeeBean = {
                ...
                roastLevel: bean.roastLevel
                  ? (bean.roastLevel as string).trim()
                  : '',
                roastDate: bean.roastDate
                  ? (bean.roastDate as string).trim()
                  : '',
                flavor:
                  Array.isArray(bean.flavor) && bean.flavor.length > 0
                    ? (bean.flavor as string[])
                    : [],
              ...
              if (bean.startDay !== undefined)
                coffeeBean.startDay = bean.startDay as number;
              if (bean.endDay !== undefined)
                coffeeBean.endDay = bean.endDay as number;
              if (bean.isFrozen !== undefined)
                coffeeBean.isFrozen = bean.isFrozen as boolean;
              if (bean.isInTransit !== undefined)
                coffeeBean.isInTransit = bean.isInTransit as boolean;
              if (bean.beanType)
                coffeeBean.beanType = bean.beanType as 'espresso' | 'filter';
              if (
                bean.blendComponents &&
                Array.isArray(bean.blendComponents) &&
                bean.blendComponents.length > 0
              ) {
                coffeeBean.blendComponents = bean.blendComponents as Array<{
                  origin?: string;
                  process?: string;
                  variety?: string;
                  percentage?: number;
        ...
        // 缓存数据
        const cacheData = {
          allBeans: allBeansData,
          beansByBrand: beansByBrandData,
        };
  ...
  const handleTouchMove = useCallback(
    ...
    [touchStartY, isAtTop, isPullRefreshing]
  ); // PULL_THRESHOLD是外部常量，不需要依赖
  ...
  const generateBeanTitle = useCallback(
    (bean: CoffeeBean, showOnlyName: boolean = false): string => {
      if (showOnlyName) {
        return bean.name;
      }
  ...
  const getFlavorPeriodStatus = useCallback(
    (flavorInfo: FlavorInfo): string => {
  ...
  const getFullNotesContent = useCallback(
    (bean: CoffeeBean) => {
  ...
  const renderBeanItem = useCallback(
    (bean: CoffeeBean, index: number, keyPrefix: string = '') => {
      const displayTitle = generateBeanTitle(
        bean,
        userSettings.showOnlyBeanName as boolean
      );
      ...
      return (
        <div
          ...
          tabIndex={0}
          role="button"
          className="focus:ring-opacity-50 cursor-pointer rounded focus:ring-2 focus:ring-blue-500 focus:outline-none"
        >
          <div className="flex gap-3">
            <div className="relative self-start">
              ...
              {(userSettings.showStatusDots as boolean) && (
                <div
                  className={`absolute -right-0.5 -bottom-0.5 h-2 w-2 rounded-full ${bean.roastDate ? getStatusDotColor(flavorInfo.phase) : 'bg-neutral-400'} border-2 border-neutral-50 dark:border-neutral-900`}
                />
              )}
            </div>
            ...
            <div className="flex flex-col gap-y-2">
              <div className={`flex flex-col justify-center gap-y-1.5`}>
                ...
                <div className="text-xs leading-relaxed font-medium tracking-wide text-neutral-600 dark:text-neutral-400">
                  <span className="inline">
                    {bean.roastDate
                      ? (userSettings.dateDisplayMode as string) ===
                        'flavorPeriod'
              ...
              {shouldShowNotes && (
                <div className="rounded bg-neutral-200/30 p-1.5 text-xs leading-tight font-medium tracking-widest whitespace-pre-line text-neutral-800/70 dark:bg-neutral-800/40 dark:text-neutral-400/85">
                  <div
                    className={
                      (userSettings.limitNotesLines as boolean)
                        ? getLineClampClass(
                            userSettings.notesMaxLines as number
                          )
                        : ''
                    }
                  >
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
          className="fixed inset-0 z-[70] mx-auto flex max-w-[500px] flex-col bg-neutral-50 dark:bg-neutral-900"
          initial={{ x: '100%' }}
          animate={{ x: 0 }}
          transition={{
            duration: 0.35,
        ...
        >
          {/* 头部导航栏 */}
          <div className="pt-safe-top flex items-center bg-neutral-50 px-4 py-4 dark:bg-neutral-900">
            {/* 返回按钮 */}
            <button
              onClick={handleClose}
            ...
            <div className="flex-1">
              ...
              >
                <input
                  ...
                  enterKeyHint="search"
                  autoComplete="off"
                  className="h-9 w-full bg-neutral-100/80 px-4 pr-12 text-xs text-neutral-800 placeholder-neutral-500 transition-colors focus:border-neutral-300 focus:bg-white focus:outline-hidden dark:bg-neutral-800/80 dark:text-neutral-200 dark:placeholder-neutral-400 dark:focus:border-neutral-600 dark:focus:bg-neutral-800"
                />
                <button
                  type="submit"
                  onClick={handleSearchButtonClick}
                  disabled={!searchQuery.trim()}
                  className="absolute top-1/2 right-1 flex h-8 w-8 -translate-y-1/2 transform items-center justify-center rounded transition-colors disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {isSearching ? (
                    <div className="h-4 w-4 animate-spin rounded-full border-2 border-neutral-400 border-t-neutral-700 dark:border-neutral-500 dark:border-t-neutral-300"></div>
                  ) : (
          ...
          {(pullDistance > 0 || isPullRefreshing) && (
            <div
              className="absolute top-24 right-0 left-0 z-10 flex items-center justify-center py-3 text-neutral-600 dark:text-neutral-400"
              style={{
          ...
          {/* 内容区域 */}
          <div
            className="pb-safe-bottom flex-1 overflow-y-auto"
          ...
          >
            <div className="px-6 pt-4">
              {searchQuery ? (
                ...
                <div className="space-y-5">
                  {searchResults.length > 0 ? (
                    searchResults.map((bean, index) =>
                      renderBeanItem(bean, index, 'search-')
                    )
                  ) : (
                ...
                <div className="space-y-6">
                  {isLoadingAll && !isPullRefreshing ? (
                    <div className="py-16 text-center text-neutral-500 dark:text-neutral-400">
                      <div className="mx-auto mb-4 h-6 w-6 animate-spin rounded-full border-2 border-neutral-400 border-t-neutral-700 dark:border-neutral-500 dark:border-t-neutral-300"></div>
                    ...
                    Object.entries(beansByBrand).map(([brandName, beans]) => (
                      <div key={brandName}>
                        {/* 品牌标题 */}
                        <div className="sticky top-0 z-10 bg-neutral-50 pb-3 dark:bg-neutral-900">
                        ...
                        <div className="space-y-5">
                          {beans.map((bean, index) =>
                            renderBeanItem(bean, index, `${brandName}-`)
                          )}
                        </div>
                      </div>
                    ))
                    ...
                    <div className="py-16 text-center text-neutral-500 dark:text-neutral-400">
                      ...
                      <p className="mt-1 text-xs opacity-60">请检查网络连接</p>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="px-6 pt-16 pb-6">
                <p className="text-center text-[10px] leading-relaxed text-neutral-300 opacity-60 dark:text-neutral-600">
                  数据来源于网络公开资源，仅供学习参考，如有异议请联系{' '}
                  <a
                    href="mailto:chuthree@163.com"
