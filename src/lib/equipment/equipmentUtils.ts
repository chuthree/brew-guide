...
export const equipmentUtils = {
     ...
     */
    getAllEquipments: (customEquipments: CustomEquipment[], equipmentOrder?: EquipmentOrder) => {
        const systemEquipments = equipmentList.map((eq) => ({ ...eq, isCustom: false }));
        const customEquipmentsWithFlag = customEquipments.map((eq) => ({ ...eq, isCustom: true }));
     ...
     * 检查是否为自定义器具
     */
    isCustomEquipment: (equipmentId: string, customEquipments: CustomEquipment[]): boolean => {
     ...
     * 从排序后的器具列表生成排序数据
     */
    generateEquipmentOrder: (allEquipments: Array<{ id: string }>): EquipmentOrder => {
        return {
    ...
    scrollToSelected: (
        ...
        const selectedElement = container.querySelector(`[data-tab="${selectedEquipment}"]`)
     ...
     * 检查滚动边框显示状态
     */
    getScrollBorderState: (container: HTMLDivElement): {
