...
export class S3Client {
  ...
  private async prepareRequest(
    ...
    if (this.isQiniu()) {
      ...
      const { requestUrl: presignedUrl, signedHeaders } =
        await this.createPresignedUrl(method, url, 60, normalizedHeaders);

      return {
        requestUrl: presignedUrl,
        headers: this.filterHeadersBySignedList(
          additionalHeaders,
          signedHeaders
        ),
      };
  ...
  private filterHeadersBySignedList(
    ...
    const normalizedSigned = new Set(
      signedHeaders.map(header => header.toLowerCase())
    );
  ...
  private async hmacSha256(
    ...
    return this.getSubtleCrypto().sign(
      'HMAC',
      cryptoKey,
      new TextEncoder().encode(data)
    );
