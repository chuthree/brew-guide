...
export class S3SyncManager {
    ...
    async sync(options: SyncOptions = {}): Promise<SyncResult> {
        ...
        try {
            ...
            console.warn('📊 同步状态检查:', {
                ...
                远程文件数: remoteMetadata ? Object.keys(remoteMetadata.files).length : 0,
                远程文件: remoteMetadata ? Object.keys(remoteMetadata.files) : [],
                远程文件详情: remoteMetadata ? Object.entries(remoteMetadata.files).map(([key, meta]) => ({
                    key,
                    hash: meta.hash.substring(0, 12),
                    size: meta.size,
                    time: new Date(meta.mtimeCli).toLocaleString()
            ...
            // 🔍 临时启用详细日志用于调试
            console.warn('🔍 同步计划详情:', {
                上传: plan.upload.map(f => ({ key: f.key, hash: f.hash.substring(0, 12), size: f.size })),
                下载: plan.download.map(f => ({ key: f.key, hash: f.hash.substring(0, 12), size: f.size })),
                冲突: plan.conflicts.map(f => ({ key: f.key, hash: f.hash.substring(0, 12), size: f.size })),
            ...
            if (plan.conflicts.length > 0 && options.conflictStrategy === ConflictStrategy.MANUAL) {
     ...
     * 执行同步计划
     */
    private async executeSyncPlan(plan: SyncPlan, result: SyncResult, options: SyncOptions = {}): Promise<void> {
        const totalOperations = plan.upload.length + plan.download.length + plan.deleteRemote.length + plan.deleteLocal.length
        ...
        if (plan.upload.length > 0) {
            for (const file of plan.upload) {
                try {
                    ...
                    if (content) {
                        const success = await this.client!.uploadFile(`${file.key}.json`, content)
                        if (success) {
                            result.uploadedFiles++
                        } else {
