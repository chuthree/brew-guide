...
const loadCSVData = async () => {
  try {
    // 使用fetch来加载CSV数据
    const [filter2025, espresso2025, filter2024, espresso2024, fenixEspresso] =
      await Promise.all([
    ...
    filterBeans2025CSV = parseCSV(filter2025) as unknown as Record<
      string,
      string
    >[];
    espressoBeans2025CSV = parseCSV(espresso2025) as unknown as Record<
      string,
      string
    >[];
    filterBeans2024CSV = parseCSV(filter2024) as unknown as Record<
      string,
      string
    >[];
    espressoBeans2024CSV = parseCSV(espresso2024) as unknown as Record<
      string,
      string
    >[];
    fenixEspressoBeansCSV = parseCSV(fenixEspresso) as unknown as Record<
      string,
      string
    >[];
  } catch (error) {
  ...
  return dataRows
    ...
    .map(row => {
      ...
      if (beanType === 'filter') {
        ...
        [序号, 品牌, 咖啡豆, 烘焙度, 克价, 喜好星值, videoEpisode, ...rest] =
          rowArray;
        处理法 = ''; // 意式豆没有处理法信息
      }
...
function parseCSVContent2024(
  ...
  return (
    validRows
      ...
      .filter(({ row }) => {
        // row is now correctly typed as an array
        ...
        if (beanType === 'filter') {
          ...
          const priceVal = String(元克 ?? '')
            .replace('没花钱', '0')
            .trim();
          const priceIsValid =
            priceVal.length > 0 && !isNaN(parseFloat(priceVal));
          const episodeExists =
            期 !== undefined && 期 !== null && String(期).trim().length > 0;
          ...
          if (
            String(品牌 ?? '').trim() === '皮爷咖啡' &&
            String(豆子 ?? '').trim() === '多明戈大街'
          ) {
            console.warn(
              'Checking simplified filter for 皮爷咖啡 多明戈大街:',
              { brandExists, nameExists, episodeExists, row }
            );
          }
      ...
      .map(({ row, originalIndex }) => {
        // Use the preserved original index from validRows mapping
        ...
        const pricePerGram =
          parseFloat(String(元克 ?? '').replace('没花钱', '0')) || 0;
...
function getLatestYear(
  ...
  const dates = [首测日期, 复测日期1, 复测日期2].filter(
    date => date && date.trim() !== ''
  );
  const years = dates
    .map(parseYearFromDate)
...
function parseFenixCSVContent(
  ...
  return dataRows
    .filter(row => {
      ...
      return (
        品牌 && 豆子名称 && 每百克价格 !== undefined && 最新评分 !== undefined
      );
    })
    .map((row, index) => {
      const [
...
export async function getBloggerBeans(
  type: 'all' | 'espresso' | 'filter' = 'all',
  ...
  try {
    if (blogger === 'fenix') {
      ...
      const espressoCSV =
        year === 2024 ? espressoBeans2024CSV : espressoBeans2025CSV;
...
export function getVideoUrlFromEpisode(
  ...
  const formattedEpisode =
    trimmedEpisode.length === 1 ? `0${trimmedEpisode}` : trimmedEpisode;
