...
export type BackupReminderInterval = typeof BACKUP_REMINDER_INTERVALS[keyof typeof BACKUP_REMINDER_INTERVALS]

/**
 * 备份提醒类型
 */
export type BackupReminderType = 'hasDataNeverBackedUp' | 'firstTimeAfterDays' | 'periodicReminder'
...
export class BackupReminderUtils {
  ...
  static async hasSignificantData(): Promise<boolean> {
    try {
      ...
      const customEquipments = customEquipmentsStr ? JSON.parse(customEquipmentsStr) : []
      const equipmentsCount = Array.isArray(customEquipments) ? customEquipments.length : 0
  ...
  static async saveSettings(settings: BackupReminderSettings): Promise<void> {
    try {
      ...
      if (typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('backupReminderSettingsChanged', {
          detail: { settings }
        }))
      }
    } catch (error) {
  ...
  static async initializeFirstUse(): Promise<void> {
    ...
    if (!settings.firstUseDate) {
      ...
      const updatedSettings: BackupReminderSettings = {
        ...settings,
        firstUseDate: now,
        nextReminderDate: this.calculateNextReminderDate(now, this.FIRST_REMINDER_DAYS)
      }
  ...
  static async markReminderShown(): Promise<void> {
    ...
    const updatedSettings: BackupReminderSettings = {
      ...
      nextReminderDate: settings.interval > 0 ?
        this.calculateNextReminderDate(now, settings.interval) : null
    }
   ...
   * 计算下次提醒日期
   */
  private static calculateNextReminderDate(fromDate: string, intervalDays: number): string {
  ...
  static async updateInterval(interval: BackupReminderInterval): Promise<void> {
    ...
    const updatedSettings: BackupReminderSettings = {
      ...settings,
      interval,
      nextReminderDate: interval > 0 ? 
        this.calculateNextReminderDate(now, interval) : null
    }
