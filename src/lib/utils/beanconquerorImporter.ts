...
function mapBrewEquipment(preparationName?: string): string {
    ...
    if (name.includes('意式') || name.includes('espresso') || name.includes('过滤手柄') || 
        name.includes('咖啡机') || name.includes('portafilter')) {
        return 'Espresso';
    }
 ...
 */
function convertBean(bean: BeanconquerorBean, remainingCapacity: number): CoffeeBean {
...
function convertBrew(
    ...
    return {
        id: brew.config?.uuid || `imported-brew-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
        timestamp,
        ...
        params: {
            ...
            ratio: brew.grind_weight && brew.brew_quantity 
    ...
    try {
        ...
        if (data.BREWS && Array.isArray(data.BREWS)) {
            for (const brew of data.BREWS) {
                const convertedBrew = convertBrew(brew, beansMap, beanUuidToNameMap, preparationsMap, millsMap);
                if (convertedBrew) {
...
export function isBeanconquerorData(jsonString: string): boolean {
    try {
        ...
        return !!(
            data &&
            typeof data === 'object' &&
            (data.BEANS || data.BREWS || data.MILL || data.PREPARATION || data.SETTINGS || data.VERSION)
        );
    } catch {
        return false;
    }
