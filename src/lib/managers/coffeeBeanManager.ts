...
export const CoffeeBeanManager = {
    ...
    try {
      ...
      if (bean.remaining) {
        ...
        if (isNaN(currentRemaining)) {
          console.warn(
            'increaseBeanRemaining: 当前剩余量格式异常',
            bean.remaining
          );
          currentRemaining = 0;
        }
      ...
      if (bean.capacity) {
        ...
        if (!isNaN(totalCapacity) && totalCapacity > 0) {
          if (finalRemaining > totalCapacity) {
            console.warn(
              `increaseBeanRemaining: 恢复后容量(${finalRemaining}g)超过总容量(${totalCapacity}g)，已限制为总容量`
            );
      ...
      if (result) {
        console.warn(
          `咖啡豆容量恢复成功: ${bean.name} +${restoreAmount}g (${currentRemaining}g -> ${formattedNewRemaining}g)`
        );
      } else {
  ...
  async updateBeanRatings(
    ...
    try {
      // 确保评分是有效的数字
      if (ratings.overallRating !== undefined && ratings.overallRating > 0) {
        // 如果bean类型未定义，设置默认值
        if (!ratings.beanType) {
            ...
            return {
              ...
              coffeeBeanInfo: {
                ...
                roastLevel:
                  ...
                  '',
                roastDate:
                  updatedBean.roastDate || note.coffeeBeanInfo?.roastDate,
              },
            };
          }
          return note;
