...
export const CustomFlavorDimensionsManager = {
    ...
    async saveFlavorDimensions(dimensions: FlavorDimension[]): Promise<void> {
        try {
            ...
            // 触发自定义事件通知其他组件
            window.dispatchEvent(new CustomEvent('flavorDimensionsChanged', {
                detail: { dimensions }
            }))
        } catch (error) {
     ...
     * 更新风味维度
     */
    async updateFlavorDimension(id: string, updates: Partial<Pick<FlavorDimension, 'label'>>): Promise<void> {
        const dimensions = await this.getFlavorDimensions()
     ...
     * 创建兼容旧格式的风味评分对象
     */
    createEmptyTasteRatings(dimensions: FlavorDimension[]): Record<string, number> {
