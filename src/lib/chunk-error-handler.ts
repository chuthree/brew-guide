...
class ChunkErrorManager {
    ...
    init() {
        ...
        try {
            // 监听全局错误
            window.addEventListener('error', this.handleError.bind(this), { passive: true });
            window.addEventListener('unhandledrejection', this.handleRejection.bind(this), { passive: true });
